"""MIT: one known initial-D1 finite sensing-divider replay using only public APM inputs."""
import argparse
import copy
import json
import os
...
def main():
    ...
    original = index['cases']['D1-half-divider']
    ...
    real = research.seal(dict(schema=research.SCHEMAS['realization'], origin='research', profile_tier='ARTIFICIAL',
        ...
        public_replay=dict(original_realization_id=original['realization_id'],
            rule='Relocated paths and public terminal-vector recipes; same saved six zero-raw units and finite sensing-divider fixture. No redraw, rebias or fresh confirmation.')))
    ...
    load=o['v(out)']/fixed['rload_ohm']
    upper=(o['v(out)']-o['v(feedback)'])/fixed['rupper_ohm']
    lower=o['v(feedback)']/fixed['rlower_ohm']
    gate=o['i(VtM2g)']
    output_kcl=avail-load-upper
    sensing_kcl=upper-lower+o['i(Vinj)']
    assert abs(sensing_kcl)<=tol['kcl_max_a'] and abs(o['i(Vinj)']+gate)<=tol['kcl_max_a']
    ...
    flows = dict(Rsource=(o['v(source)']-o['v(inp)'], -o['i(Vin)'], fixed['rsource_ohm']),
        Rload=(o['v(out)'], avail-upper, fixed['rload_ohm']),
        Rupper=(o['v(out)']-o['v(feedback)'],avail-load,fixed['rupper_ohm']),
        Rlower=(o['v(feedback)'],avail-load-gate,fixed['rlower_ohm']),
    ...
    icap = -ac['i(VtM2d)']-ac['i(VtM4d)']-ac['v(out)']/fixed['rload_ohm']-(ac['v(out)']-ac['v(feedback)'])/fixed['rupper_ohm']
    ...
    result = dict(schema='learning.initial-d1-feedback-divider.public-replay.v1', status='PASS',
        role='One previously exposed finite sensing-divider initial-D1 reproduction; not new scientific confirmation.',
        ...
        differences=differences, output_v=o['v(out)'], error_v=o['v(out)']-2*o['v(source)'],
        gain_one_khz=[float(h[0].real), float(h[0].imag)], bandwidth=bandwidth(f, h),
        maximum_MOS_KCL_a=kcl, maximum_terminal_span_v=span,
        output_node_KCL_a=output_kcl, input_node_KCL_a=input_kcl,sensing_node_KCL_a=sensing_kcl,
        ...
        passive_readback='Measured DC V/I for all five resistors and complex AC current/voltage for Cload; no copied expected values as observations.',
        limits='One stationary gain-of-two divider OP/AC example on the same core; finite function and resistor load differ from the unity buffer. No transient, receiving-load range qualification, noise population, regulator or independent scientific replication.')
