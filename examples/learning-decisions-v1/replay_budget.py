"""MIT: one public native replay of the supported 12-pF deadline example.
...
import numpy as np
from measurement_v1 import columns, final_entry, bandwidth, integrate
from load_budget import ROOT, read, sha, calculate, deadline_result
...
def main():
    ...
    checked=calculate();cfg=read(ROOT/'data/interface-inputs.json');plan=read(ROOT/'load-budget-plan.json')
    name='D1-budget-12p';case=cfg['cases'][name];condition=case['case']
    with np.load(ROOT/'data'/case['array_file'],allow_pickle=False) as z:
        saved={name:{k:z[k] for k in z.files}}
    binary_version=subprocess.check_output([str(a.binary),'--version'],text=True)
    assert 'ngspice-47' in binary_version, binary_version
    ...
    real=research.seal(dict(schema=research.SCHEMAS['realization'],origin='research',profile_tier='ARTIFICIAL',status='RESOLVED',
        ...
        devices=case['physical_devices'],public_replay=dict(original_realization_id=case['original_realization_id'],
            case=name,rule='Relocated paths and public terminal-vector recipes; exact saved six zero-raw units and 12-pF circuit, no redraw or trim.')))
    ...
    expected=checked['cases'][name]['measured']
    for edge,origin,target,horizon in [('rise',.50625e-6,high,4.5e-6),('fall',4.50625e-6,low,8.5e-6)]:
        ...
        assert deadline_result(item,plan['deadline']['seconds'])==checked['cases'][name]['edges'][edge]['status']
        entries[edge]=item
    result=dict(status='PASS',case=name,apm_commit=cfg['apm_commit'],new_run_id=report['run_id'],
        ngspice_version=binary_version.strip(),binary_sha256=sha(a.binary),
        deadline_s=plan['deadline']['seconds'],deadline_result=checked['cases'][name]['deadline_result'],
        ...
        limits='One nominal 12-pF initial-D1 OP/AC/standard-step replay against the predeclared illustrative deadline. No Local cohort, full pass-gate charge, LDO, old selected-D1 transfer or independent scientific replication.')
