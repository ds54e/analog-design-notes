"""MIT: one known initial-D1 100kohm-load replay using only public APM inputs."""
import argparse
import copy
import json
import os
...
def main():
    ...
    original = index['cases']['D1-100k-load']
    ...
    real = research.seal(dict(schema=research.SCHEMAS['realization'], origin='research', profile_tier='ARTIFICIAL',
        ...
        public_replay=dict(original_realization_id=original['realization_id'],
            rule='Relocated paths and public terminal-vector recipes; same saved six zero-raw units and 100kohm-load fixture. No redraw, rebias or fresh confirmation.')))
    ...
    result = dict(schema='learning.initial-d1-dc-load.public-replay.v1', status='PASS',
        role='One previously exposed 100kohm-load initial-D1 reproduction; not new scientific confirmation.',
        ...
        limits='One stationary 100kohm-load OP/AC example on the same core. No transient, receiving-load range qualification, noise population, regulator or independent scientific replication.')
