"""MIT: finite initial-core common-mode comparison; pure saved-data analysis."""
import argparse
import csv
import hashlib
import io
...
def inputs(source_zip):
    ...
    with zipfile.ZipFile(source_zip) as z:
        ...
        cfg = json.loads(z.read('example/data/interface-inputs.json'))
        priors = {}
        for name in ('C-source-0p70', 'D1-5p'):
            case = cfg['cases'][name]
            assert case['physical_devices'] == plan['physical_units']
            with np.load(io.BytesIO(z.read('example/data/'+case['array_file'])), allow_pickle=False) as data:
                arrays = {k: data[k] for k in data.files}
            index = 0 if name.startswith('C') else 1
            _, values = columns(case['analyses'][index]['recipe'], arrays['analysis'+str(index)])
            op = {k: float(v[0]) for k, v in values.items()}
            expected = read(ROOT/'data/prior.json')['cases'][name]['observed_op']
            assert op == expected, 'Prior OP differs from the published selected observations'
            priors[name] = dict(case=case, arrays=arrays, op=op)
    return plan, priors
...
def analyze(source_zip, observations, output):
    assert not output.exists()
    plan, priors = inputs(source_zip)
    index = read(observations/'index.json')
    assert index['plan_sha256'] == sha(ROOT/'plan.json')
    results = {}
    for name, case in plan['cases'].items():
        entry = index['cases'][name]
        assert sha(observations/entry['file']) == entry['sha256']
        with np.load(observations/entry['file'], allow_pickle=False) as z:
            arrays = {k: z[k] for k in z.files}
        results[name] = measure_case(name, case, arrays, plan)
    prior = read(ROOT/'data/prior.json')['cases']
    dc = results['D1-high-command']
    cc = results['C-high-common-mode']
    comparison = dict(
        old_clamp_net_a=prior['C-source-0p70']['net_after_load_a'],
        new_clamp_net_a=cc['net_after_resistor_a'],
        clamp_net_change_a=cc['net_after_resistor_a']-prior['C-source-0p70']['net_after_load_a'],
        old_d1_central_output_v=prior['D1-5p']['output_v'],
        old_d1_central_gain=prior['D1-5p']['gain_one_khz'],
        local_output_screen_v=prior['D1-5p']['linear_output_screen_at_0p7_v'],
        observed_high_output_v=dc['output_v'],
        output_screen_residual_v=dc['output_v']-prior['D1-5p']['linear_output_screen_at_0p7_v'])
    result = dict(schema='learning.initial-d1-common-mode.measure.v1', status='NUMERICAL_CHECKS_PASS',
        plan_sha256=sha(ROOT/'plan.json'), cases=results, comparison=comparison,
        limits=plan['limits'])
    output.mkdir(parents=True)
    (output/'summary.json').write_text(json.dumps(result, indent=2, sort_keys=True)+'\n')
    keys = ['case', 'input_common_mode_v', 'input_difference_v', 'output_v', 'tail_v', 'tail_vsd_v',
            'p2_vsg_v', 'p2_vsd_v', 'tail_delivered_a', 'source_branch_a', 'sink_branch_a',
            'net_after_resistor_a', 'supply_delivered_a']
    with (output/'operating-points.csv').open('w') as f:
        writer = csv.DictWriter(f, keys)
        writer.writeheader()
        for name, row in results.items():
            writer.writerow({k: name if k == 'case' else row[k] for k in keys})
...
def main():
    ...
    a = p.parse_args()
    analyze(a.source_zip, a.observations, a.output)
